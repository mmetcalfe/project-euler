...
# (Published on Friday, 3rd December 2004, 06:00 pm; Solved by 5664)
# 
#     In the game, Monopoly, the standard board is set up in the
# 	following way:
# 
#              GO      A1     CC1      A2      T1      R1      B1
# 	CH1      B2      B3    JAIL
#          H2              C1
#          T2              U1
#          H1              C2
...
# 	clockwise direction. Without any further rules we would expect to
# 	visit each square with equal probability: 2.5%. However, landing on
# 	G2J (Go To Jail), CC (community chest), and CH (chance) changes
# 	this distribution.
# 
...
# 	When a player lands on CC or CH they take a card from the top of
# 	the respective pile and, after following the instructions, it is
# 	returned to the bottom of the pile. There are sixteen cards in each
# 	pile, but for the purpose of this problem we are only concerned
# 	with cards that order a movement; any instruction not concerned
# 	with movement will be ignored and the player will remain on the
# 	CC/CH square.
# 
...
# 	Square 24, and GO (3.09%) = Square 00. So these three most popular
