...
main = do
    let greatestPrime = last $ takeWhile (<10^6) primes -- 999983
        longestSeq = fst $ last $  takeWhile ((<10^6) . snd) $ map (\x -> (x, (sum . flip take primes) x)) [1..]
    print greatestPrime
    print longestSeq
    let sums = map (flip consecutiveSums (filter (<= greatestPrime) primes)) [21..longestSeq]
